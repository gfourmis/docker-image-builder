...
```bash
    ...
    #Nota:   
    #- Solo este usuario podrá hacer algo sobre la base de datos.
...
```
# 3. Testeo de autenticación, roles y permisos
...
```bash
    >db.getUsers()
    #nota: 
    #- En el arreglo debe aparecer el anterior usuario, si no aparece es que no se podrá autenticar.
...
```bash
    ...
    #nota: 
    #Identicar que en cada documento el atributo "_id" tiene la base de datos concatenado con el usuario:
        #admin.gfAdmin
        #dev-sp1.devUser
        #.
        #.
...
```bash
    mongo --host 192.168.1.130 -u devUser -p 123456 \
    --authenticationDatabase admin

    mongo --host 192.168.1.130 -u prdUser -p 123456 \
    --authenticationDatabase admin
    
    #nota: 
    #- Puede que algunos de estos comandos fallen, queda al entendimiento como resolver.
    #- IMPORTANTE despues de autenticarse, igualmente hay que usar "use database"  
```

# 4. Realizando backups y restore        
PARA LOS BACKUPS
```bash
    >use admin
    >db.auth('gfAdmin','123456')
...
#nota: 
    #- Esto se puede replicar en cualquier base de datos para realizar los backups y restore
